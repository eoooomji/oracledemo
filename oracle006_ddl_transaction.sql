...
-- salary컬럼의 값을 8000으로 업데이트 하세요.
UPDATE emp01
SET salary = 8000
WHERE first_name = 'candy';
/* salary 값이 5000에서 8000으로 수정 */
...
-- emo01테이블에서 first_name컬럼의 값이 'candy'일때 삭제하세요.
DELETE FROM emp01
WHERE first_name = 'candy';
...
2 트랜잭션 안전성 확보를 위한 ACID
  ...
  3) 트랜잭성 구성요소   
    Atomicity(원자성):
     - 트랜잭션은 한 개 이상의 동작을 논리적으로 한 개 작업단위(single unit of work)로서 
       분해가 불가능한 최소의 단위(일부분만 적용할 수 없다.)
...
-- user_tables : 현재 사용자가 접근할 수 있는 table
SELECT * FROM emp01;
SELECT * FROM hr.emp01;
SELECT * FROM user_tables; /* 데이터 테이블 사전 */
...
-- 스키마
-- DDL : DDL명령어는 실행 시 자동 커밋이 된다. (CREATE) 
CREATE TABLE emp02(
    emp01_id number primary key,  /* primary = NOT NULL + UNIQUE(중복된 값과 NULL이 들어갈 수 없다.) */
    first_name varchar2(50), 
    /* varchar2(50) 
    한글 한 문자당 3바이트. 총 18문자 할당 가능
    varchar2는 가변, char는 고정 
    */
    salary number
);
