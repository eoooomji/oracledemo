...
1)EMPLOYEES 테이블에서 입사한 달(hire_date) 별로 인원수를 조회하시오 . 
  <출력: 월        직원수   >
SELECT to_char(hire_date,'mm') AS "월", count(*)|| '명' AS "직원수"
FROM employees
...
3)각 부서 이름 별로 2005년 이전에 입사한 직원들의 인원수를 조회하시오.
 <출력 :    부서명		입사년도	인원수  >
SELECT d.department_name AS "부서명", to_char(e.hire_date, 'yyyy') AS "입사년도" ,count(*) AS "직원수"
FROM employees e, departments d
WHERE e.department_id = d.department_id(+)
GROUP BY department_name, to_char(hire_date, 'yyyy')
HAVING to_char(e.hire_date,'yyyy') <= '2005'
-- ORDER BY "입사년도" DESC;
ORDER BY 2 DESC; -- 인덱스로도 가능하다.
